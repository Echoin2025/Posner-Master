...
let expInfo = {
  participant: "",
  session: "001",
  // optional demographic fields shown in the pre-experiment dialog
  // gender is presented as a dropdown; age is optional free text
  gender: ["Male", "Female", "Other", "Prefer not to say"],
  age: "",
};

// how many trials to run per test
const RUN_TRIALS = 25;
...
async function updateInfo() {
  ...
  // age: optional, but if supplied must be an integer between 0 and 120
  if (typeof expInfo["age"] !== "undefined" && expInfo["age"] !== "") {
    const ageNum = Number(expInfo["age"]);
    if (
      Number.isNaN(ageNum) ||
      ...
      window.alert(
        "年龄字段必须为空或为 0 到 120 之间的整数；已清空该字段，请在对话框中重新填写（可选）。"
      );
      expInfo["age"] = "";
    }
  }
  // gender: if present, ensure it is one of the allowed options
  if (typeof expInfo["gender"] !== "undefined" && expInfo["gender"] !== "") {
    const allowedGenders = ["Male", "Female", "Other", "Prefer not to say"];
    if (!allowedGenders.includes(expInfo["gender"])) {
      window.alert("性别选择无效，已重置为未选择。");
      expInfo["gender"] = "";
    }
  }
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function () {
    ...
    trials = new TrialHandler({
      psychoJS: psychoJS,
      // ensure we have enough repeats to draw RUN_TRIALS samples; we'll schedule only RUN_TRIALS below
      nReps: 10,
    ...
    // Schedule exactly RUN_TRIALS trials (stop early if trial handler would produce more)
    let scheduledTrials = 0;
    for (const thisTrial of trials) {
      if (scheduledTrials >= RUN_TRIALS) {
        break;
      }
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    trial_counter.setText(
      (trials.thisN + 1).toString() + "/" + RUN_TRIALS.toString()
    );
    // keep track of which components have finished
    trialComponents = [];
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    // store data for psychoJS.experiment (ExperimentHandler)
    if (mouse.x) {
      psychoJS.experiment.addData("mouse.x", mouse.x[0]);
    }
    if (mouse.y) {
      psychoJS.experiment.addData("mouse.y", mouse.y[0]);
    }
    if (mouse.leftButton) {
      psychoJS.experiment.addData("mouse.leftButton", mouse.leftButton[0]);
    }
    if (mouse.midButton) {
      psychoJS.experiment.addData("mouse.midButton", mouse.midButton[0]);
    }
    if (mouse.rightButton) {
      psychoJS.experiment.addData("mouse.rightButton", mouse.rightButton[0]);
    }
    if (mouse.time) {
      psychoJS.experiment.addData("mouse.time", mouse.time[0]);
    }
...
function feedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    rtfb.setText(
      "Response time: " +
        Number.parseInt(mouse.time.slice(-1)[0] * 1000).toString() +
        "ms"
    );
...
function finishRoutineBegin(snapshot) {
  return async function () {
    ...
    endfb =
      "Your response time for congruent trials was " +
      Number.parseInt(valid_av * 1000).toString() +
      "ms\nYour response time for incongruent targets was " +
      Number.parseInt(invalid_av * 1000).toString() +
      "ms";
